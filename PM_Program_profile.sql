...
select COUNT(DISTINCT LOCATION)LOCATION,SiteId  from PMO_3410012
GROUP BY SiteId 


---------------------------TOTAL PM WO COUNT----------------------------------

select   count (distinct WONUM)PM_WOCOUNT,A.SITEID from [dbo].[PMO_3410012]  a left join Client_Workorder w
on a.ASSETNUM=w.ASSETNUM
where  DEV_WORKTYPE='pm' and w.Year='31-MAR-2022'and  
a.statusid  in ('OPERATING','ACTIVE')
GROUP  BY A.SITEID
...
----------------ASSET_COUNT_BY_CRITICALITY

SELECT COUNT(AssetNum),ECRRanking FROM PMO_3410012
WHERE SiteId='DAF'
GROUP BY ECRRanking
